...
static void test_count_bits(void)
{
  ...
  for (int t = 0; t < num_trials; t++) {
    memset(arr, 0, sizeof(arr));
    /* pick N distinct random positions using Fisher-Yates on a position list */
    int positions[TOTAL_BITS];
    for (int i = 0; i < TOTAL_BITS; i++)
      positions[i] = i;
    int n = 1 + rand() % (TOTAL_BITS);  /* how many bits to set */
    for (int i = 0; i < n; i++) {
      int j = i + rand() % (TOTAL_BITS - i);
...
static void test_count_bits64_with_mask(void)
{
  /* --- zero value --- */
  ...
  int num_trials = 5;
  for (int t = 0; t < num_trials; t++) {
    int start = rand() % 64; /* at least 1 bit of room */
    int num = 1 + rand() % (64 - start);
    int window_end = start + num;
    /* Fisher-Yates over window positions to pick exactly n distinct bits */
    int positions[64];
    for (int i = 0; i < num; i++)
      positions[i] = start + i;
    int n = 1 + rand() % num;
    for (int i = 0; i < n; i++) {
      int remaining = num - i;
      int j = i + (remaining > 1 ? rand() % remaining : 0);
      int tmp = positions[i];
