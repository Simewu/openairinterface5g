...
# GEO-5G Digital Twin Tutorial: Ray-Tracing Channel Emulator with OAI 5G Stack
...
This tutorial describes how to set up and run the GEO-5G digital twin testbed at EURECOM. The goal is to reproduce realistic wireless experiments by combining:

- **3D Ray-Tracing Channel Emulation**: a ray-tracing propagation simulator that generates realistic multipath channel taps from the EURECOM environment model.
...
**Ray-Tracing Channel Emulator** acts as the central channel simulator. It reads a scene configuration describing the EURECOM 3D environment, runs ray-tracing to compute multipath propagation taps, and distributes these taps over IPC sockets to the gNB and UE.
...
**OAI CN5G** is the 5G core network running in Docker containers. It includes the standard NFs (AMF, SMF, UPF, NRF, AUSF, UDM, UDR) plus a custom LMF image built from the `fix-unit-mismatch` branch, which supports UE location estimation via the `nlmf-loc` API.
